% Adopted_num（AUTOINPUT(31)）を固定


function [C,W,U,V,X,XBFR,XMT,GETA,GET] = SVD_MT_190502(ITSKP,IT,A,B,M,N,MP,NP,LTikh,...
...
%
    ...
    if (K  <= NAPB)
	    fprintf(fid101,'%d %d\r\n',B(K),C(K)); %B：シグナルベクトル、C：再構成値（センサ位置における）
	    fprintf(fid201,'%d %d\r\n',BSNSR(K),C(K)+FC(K));  %BSNSR：FFDAT、C：再構成値（センサ位置における）、FC：コイル電流寄与
    else
        if(K <= NAPB+NFLX)
	        fprintf(fid102,'%d %d\r\n', B(K)-XGETA,C(K)); % ! B(K),C(K)共に下駄が差し引かれている。　B：シグナルベクトル、C：再構成値（センサ位置における）
	        fprintf(fid202,'%d %d\r\n',FLXLP(K-NAPB),C(K)+FC(K)+GETA);% ! 測定値 FLXLP() には下駄が含まれる。　FLXLP：フラックスループシグナル、C+FC＋GETA：純粋な再構成値（コイル電流寄与、下駄を含む）
        end
    end
