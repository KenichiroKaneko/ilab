...
function [FC, BR, BZ, PSIFLX, PSIC, AA, FF] = FORM(PARAM, AA, FF, IT, OLDGT, GETA, ExtCOIL, SENSOR_TPRB, SENSOR_NPRB, SENSOR_FLXLP, CCSDAT, WALL)
    ...
    FF(1 + NAPB:NFLX + NAPB) = FF(1 + NAPB:NFLX + NAPB) - GETA + OLDGT; %! 要再確認
    FF(1 + NAPB + NFLX:sum(NCCN) + NAPB + NFLX) = FF(1 + NAPB + NFLX:sum(NCCN) + NAPB + NFLX) - GETA + OLDGT; %    ! 要再確認
end
