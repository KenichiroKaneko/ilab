...
hold on
% psi_orig = load('psi_C_03C_finemesh');
...
function [error, psi] = cal_flux(param, psi, mu_jt)
    ...
    contour(rr, zz, psi * 1000, v, 'k')
    hold on
    contour(rr, zz, psi_old * 1000, v, 'r')
    title('flux');
...
function [param, psi] = cal_initial_flux(param, psi)
    ...
    switch param.Sw_init
        ...
        case 3
            ReadIn_data(psi, "outputfiles/flux.txt");
        otherwise
...
function WriteOut_data(psi, filename)
    writematrix(psi, '2020年12月16日gs3_2033_602/Delimiter', 'Delimiter', 'tab');
end
