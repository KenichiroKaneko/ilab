...
%% EDDYP!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%% EDDYP!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
function EDDYP(FFOUT, PARAM, CONFIG, SENSOR_TPRB, SENSOR_NPRB, SENSOR_FLXLP, CCSDAT, WALL)
    DISI = zeros(1, 594);
    EDI = zeros(1, 594);
    ...
    if (NONC <= 0) %   GOTO 507
        ...
        if CONFIG.DataType == 0
            % 数値解で実行するときはReferenceを読み込む
            fid42 = fopen([PARAM.input_file_directory '/jeddy.txt'], 'r');

        else
            % 実験値のときはエラーがでるのでとりあえずReferenceを読み込む
            fid42 = fopen('CCS_dataset_gs/UTST_numel_2033/jeddy.txt', 'r');
        end
